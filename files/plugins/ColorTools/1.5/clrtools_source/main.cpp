...
int histoRunProc(const FilterActivation *fa, const FilterFunctions *ff);
int drawChart(const FilterActivation *fa, const FilterFunctions *ff, int cd, int heightOffset, float scaleF);
int drawWFM(const FilterActivation *fa, const FilterFunctions *ff, int cd, int heightOffset, float scaleF);
int histograms(const FilterActivation *fa, const FilterFunctions *ff); 
int wfm(const FilterActivation *fa, const FilterFunctions *ff); 
int vectorScope(const FilterActivation *fa, const FilterFunctions *ff); 
int hotPixels(const FilterActivation *fa, const FilterFunctions *ff);
int showChannels(const FilterActivation *fa, const FilterFunctions *ff);
int histoStartProc(FilterActivation *fa, const FilterFunctions *ff);
int histoEndProc(FilterActivation *fa, const FilterFunctions *ff);
int MyInitProc(FilterActivation *fa, const FilterFunctions *ff);
long histoParamProc(FilterActivation *fa, const FilterFunctions *ff);
int histoConfigProc(FilterActivation *fa, const FilterFunctions *ff, HWND hwnd);
void histoStringProc(const FilterActivation *fa, const FilterFunctions *ff, char *str);
void histoScriptConfig(IScriptInterpreter *isi, void *lpVoid, CScriptValue *argv, int argc);
bool histoFssProc(FilterActivation *fa, const FilterFunctions *ff, char *buf, int buflen);
double gc(double val, double gamma, bool mode);
Pixel32 hot(Pixel32 p, const FilterActivation *fa);
void xRectFill(const FilterActivation *fa, PixCoord x, PixCoord y, PixDim dx, PixDim dy, Pixel32 c);
...
ScriptFunctionDef histo_func_defs[]={
    { (ScriptFunctionPtr)histoScriptConfig, "Config", "0iiiiiiiiiiiii" },
    { NULL },
};

CScriptObject histo_obj={
...
static FilterDefinition *fd_histo;

///////////////////////////////////////////////////////////////////////////

int __declspec(dllexport) __cdecl VirtualdubFilterModuleInit2(FilterModule *fm, const FilterFunctions *ff, int& vdfd_ver, int& vdfd_compat) {
    if (!(fd_histo = ff->addFilter(fm, &filterDef_histo, sizeof(FilterDefinition))))
        return 1;
...
int SaveKey(LPTSTR lpClass, LPTSTR lpData)
{
    LPCTSTR lpSubKey = settings_key;
    HKEY keyHandle;
    long rtn =0;
    DWORD lpdw;
    int aLen = 0; 
...
//======= Draw Histogram ================
int drawChart(const FilterActivation *fa, const FilterFunctions *ff, int Xcd, int heightOffset, float scaleF) {
       ...
       case WM_INITDIALOG:
...

            {
              ...
              if(hwndPreview && mfd->ifp){
                ...
                mfd->ifp->InitButton(hwndPreview);
              }
            }
        ...
        case WM_COMMAND:
			...
			case AFX_IDC_HOTCHANGE:
				if (HIWORD(wParam) == CBN_SELCHANGE) {
					mfd->hotFix = SendDlgItemMessage(hdlg, AFX_IDC_HOTCHANGE, CB_GETCURSEL, 0, 0);
				}
        redo_frame(mfd,hdlg);
				return TRUE;
 			...
 			case AFX_IDC_MAXIRE:
				...
				if (HIWORD(wParam) == CBN_SELCHANGE) {
					i = SendDlgItemMessage(hdlg, AFX_IDC_MAXIRE, CB_GETCURSEL, 0, 0);
				}
...
int histoConfigProc(FilterActivation *fa, const FilterFunctions *ff, HWND hwnd) {
    MyFilterData *mfd = (MyFilterData *)fa->filter_data;
    mfd->ifp = fa->ifp;
	int rtn = DialogBoxParam(fa->filter->module->hInstModule,
            MAKEINTRESOURCE(IDD_FILTER_TUTORIAL), hwnd,
            (DLGPROC) histoConfigDlgProc, (LPARAM)fa->filter_data);
    mfd->ifp = 0;
...
Pixel32 hot(Pixel32 p, const FilterActivation *fa) {
	...
	if(mfd->hotFix==0) {
		...
		fy = (double)y / SCALEhp;
		fc = hypot((double)i / SCALEhp, (double)q / SCALEhp);
